...
fn getThreshold(image: Gray8Image, x: usize, y: usize, value: f32) f32 {
    const factor = 10.0;
    ...
    const threshold = -(highPassValue * entropy) * factor + 0.5;
    return threshold;
}

fn quantize(image: Gray8Image, x: usize, y: usize, value: f32) f32 {
    ...
    const threshold = getThreshold(image, x, y, value);
    const modulatedThreshold = threshold + random * modulator;
