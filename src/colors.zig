// Adapted from https://github.com/xav/Grapefruit/blob/master/grapefruit.py

const _oneThird = 1.0 / 3.0;
const _srgbGammaCorrInv = 0.03928 / 12.92;
const _sixteenHundredsixteenth = 16.0 / 116.0;
...
pub const WhiteReference = struct {
    // Taken from https://en.wikipedia.org/wiki/Standard_illuminant#White_points_of_standard_illuminants

    /// 2° (CIE 1931) - 2856K, Incandescent / Tungsten
    const std_A       = .{ 1.098470, 1.000000, 0.355823 };

    /// 2° (CIE 1931) - 4874K, Direct sunlight at noon (obsolete)
    const std_B       = .{ 0.990927, 1.000000, 0.853133 };

    /// 2° (CIE 1931) - 6774K, Average / North sky Daylight (obsolete)
    const std_C       = .{ 0.980706, 1.000000, 1.182250 };

    /// 2° (CIE 1931) - 5003K, Horizon Light
    const std_D50     = .{ 0.964212, 1.000000, 0.825188 };

    /// 2° (CIE 1931) - 5503K, Mid-morning / Mid-afternoon Daylight
    const std_D55     = .{ 0.956797, 1.000000, 0.921481 };

    /// 2° (CIE 1931) - 6504K, Noon Daylight (Television, sRGB color space)
    const std_D65     = .{ 0.950429, 1.000000, 1.088900 };

    /// 2° (CIE 1931) - 7504K, North sky Daylight
    const std_D75     = .{ 0.949722, 1.000000, 1.226390 };

    /// 2° (CIE 1931) - 5454K, Equal energy
    const std_E       = .{ 1.000000, 1.000000, 1.000000 };

    /// 2° (CIE 1931) - 6430K, Daylight Fluorescent
    const std_F1      = .{ 0.928336, 1.000000, 1.036650 };

    /// 2° (CIE 1931) - 4230K, Cool White Fluorescent
    const std_F2      = .{ 0.991447, 1.000000, 0.673159 };

    /// 2° (CIE 1931) - 3450K, White Fluorescent
    const std_F3      = .{ 1.037530, 1.000000, 0.498605 };

    /// 2° (CIE 1931) - 2940K, Warm White Fluorescent
    const std_F4      = .{ 1.091470, 1.000000, 0.388133 };

    /// 2° (CIE 1931) - 6350K, Daylight Fluorescent
    const std_F5      = .{ 0.908720, 1.000000, 0.987229 };

    /// 2° (CIE 1931) - 4150K, Lite White Fluorescent
    const std_F6      = .{ 0.973091, 1.000000, 0.601905 };

    /// 2° (CIE 1931) - 6500K, D65 simulator, Daylight simulator
    const std_F7      = .{ 0.950172, 1.000000, 1.086300 };

    /// 2° (CIE 1931) - 5000K, D50 simulator, Sylvania F40 Design 50
    const std_F8      = .{ 0.964125, 1.000000, 0.823331 };

    /// 2° (CIE 1931) - 4150K, Cool White Deluxe Fluorescent
    const std_F9      = .{ 1.003650, 1.000000, 0.678684 };

    /// 2° (CIE 1931) - 5000K, Philips TL85, Ultralume 50
    const std_F10     = .{ 0.961735, 1.000000, 0.817123 };

    /// 2° (CIE 1931) - 4000K, Philips TL84, Ultralume 40
    const std_F11     = .{ 1.008990, 1.000000, 0.642617 };

    /// 2° (CIE 1931) - 3000K, Philips TL83, Ultralume 30
    const std_F12     = .{ 1.080460, 1.000000, 0.392275 };

    /// 10° (CIE 1964) - 2856K, Incandescent / Tungsten
    const sup_A       = .{ 1.111420, 1.000000, 0.351998 };

    /// 10° (CIE 1964) - 4874K, Direct sunlight at noon (obsolete)
    const sup_B       = .{ 0.991778, 1.000000, 0.843493 };

    /// 10° (CIE 1964) - 6774K, Average / North sky Daylight (obsolete)
    const sup_C       = .{ 0.972857, 1.000000, 1.161450 };

    /// 10° (CIE 1964) - 5003K, Horizon Light
    const sup_D50     = .{ 0.967206, 1.000000, 0.814280 };

    /// 10° (CIE 1964) - 5503K, Mid-morning / Mid-afternoon Daylight
    const sup_D55     = .{ 0.957967, 1.000000, 0.909253 };

    /// 10° (CIE 1964) - 6504K, Noon Daylight (Television, sRGB color space)
    const sup_D65     = .{ 0.948097, 1.000000, 1.073050 };

    /// 10° (CIE 1964) - 7504K, North sky Daylight
    const sup_D75     = .{ 0.944171, 1.000000, 1.206430 };

    /// 10° (CIE 1964) - 5454K, Equal energy
    const sup_E       = .{ 1.000000, 1.000000, 1.000000 };

    /// 10° (CIE 1964) - 6430K, Daylight Fluorescent
    const sup_F1      = .{ 0.947913, 1.000000, 1.031910 };

    /// 10° (CIE 1964) - 4230K, Cool White Fluorescent
    const sup_F2      = .{ 1.032450, 1.000000, 0.689897 };

    /// 10° (CIE 1964) - 3450K, White Fluorescent
    const sup_F3      = .{ 1.089680, 1.000000, 0.519648 };

    /// 10° (CIE 1964) - 2940K, Warm White Fluorescent
    const sup_F4      = .{ 1.149610, 1.000000, 0.409633 };

    /// 10° (CIE 1964) - 6350K, Daylight Fluorescent
    const sup_F5      = .{ 0.933686, 1.000000, 0.986363 };

    /// 10° (CIE 1964) - 4150K, Lite White Fluorescent
    const sup_F6      = .{ 1.021480, 1.000000, 0.620736 };

    /// 10° (CIE 1964) - 6500K, D65 simulator, Daylight simulator
    const sup_F7      = .{ 0.957797, 1.000000, 1.076180 };

    /// 10° (CIE 1964) - 5000K, D50 simulator, Sylvania F40 Design 50
    const sup_F8      = .{ 0.971146, 1.000000, 0.811347 };

    /// 10° (CIE 1964) - 4150K, Cool White Deluxe Fluorescent
    const sup_F9      = .{ 1.021160, 1.000000, 0.678256 };

    /// 10° (CIE 1964) - 5000K, Philips TL85, Ultralume 50
    const sup_F10     = .{ 0.990012, 1.000000, 0.831340 };

    /// 10° (CIE 1964) - 4000K, Philips TL84, Ultralume 40
    const sup_F11     = .{ 1.038200, 1.000000, 0.655550 };

    /// 10° (CIE 1964) - 3000K, Philips TL83, Ultralume 30
    const sup_F12     = .{ 1.114280, 1.000000, 0.403530 };
};

// The default white reference, use 2° Standard Observer, D65 (daylight)
const _DEFAULT_WREF = WhiteReference.std_D65;


const NAMED_COLOR = struct {
...
/// >>> rgb_to_hsl(1, 0.5, 0)
///     (30.0, 1.0, 0.5)
pub fn rgb_to_hsl(r: f32, g: f32, b: f32) [3]f32 {

    const minVal = @min(r, @min(g, b));  // min RGB value
    const maxVal = @max(r, @max(g, b));  // max RGB value
    ...
    const d = maxVal - minVal;  // delta RGB value
    const s = if (l < 0.5) d / (maxVal + minVal) else  d / (2.0 - d);
